...
- **行 diff 换成有界 Myers（#190）**：`genDiff` 原用 jsdiff 的 `diffLines`（全文件 Myers + 逐 token 对象，无上界），现换成**基于行哈希的有界 Myers**（单遍 + trace）：公共前缀/后缀直接配对、距离超 `DEFAULT_MAX_D`（256）就二分重试、再超才退化为整块替换并置 `degraded`。语义不失：**13 个形状**（尾换行边界、整文件重写、空侧、重复行、重排式编辑……）在同一进程里与 jsdiff **逐字段对拍全绿**（`diff` 字符串 / `rows` / `servedRows` / `firstChangedLine`）；`genDiff` 保留 `partsFor` 缝供后续替代实现继续对拍。实测 80 万行：144–162 MB / ~0.42 s，与 jsdiff 同量级但**复杂度有上界**（最坏情况不再无界膨胀）。EDIT 卡片的 `structuredPatch`（407 MB）仍待换，见后续票。
