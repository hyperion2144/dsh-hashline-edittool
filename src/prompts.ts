...
export const EDIT_GUIDANCE: ToolGuidance = {
	...
	lines: [
		...
		"`edit`: `anchor_start` is required and anchors the FIRST line of the range; `anchor_end` anchors the LAST line — REQUIRED for `replace` (a single-line replace passes the same anchor twice). `op:\"ins\"` accepts ONLY `anchor_start` — the insert lands AFTER that line; `anchor_end` is rejected.",
