...
if (ctx.plugins.length !== 4) throw new Error(`expected 4 sub-plugins, got ${ctx.plugins.length}`);
...
const expected = ["read:-1:conversation", "edit:-1:conversation", "grep:-1:conversation", "write:-1:conversation"];
...
if (ctx.slots.injectCalls.filter((key) => key === "tool.call.toolview").length !== 4) {
	throw new Error("expected all four toolviews to inject the tool.call.toolview declaration");
}
console.log("bundle evaluation OK:");
console.log("  - factory registered under", registered[0].id);
console.log("  - exports {inject, apply} mount 4 toolview plugins");
