...
- **Highlight** is a semantic `<mark>` painted with `--dsw-alias-state-warn-tertiary` (the palette's only warm tier; amber-100/amber-900), with `color` explicitly inherited — the UA sheet pairs `mark`'s light background with black text, which is unreadable under a dark theme.
- **Tab bar**: one tab per file, **including the single-file case**; labels are the meta paths (unique within a result set), ellipsised with the full path as `title`; the bar scrolls horizontally on one line and never wraps; the first file is active; switching resets the fold and the copy flash. Full `tablist`/`tab`/`tabpanel` semantics with `ArrowLeft`/`ArrowRight` switching.
