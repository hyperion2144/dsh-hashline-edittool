...
| dsh 版本 | 插件版本 | 说明 |
| --- | --- | --- |
| **0.1.6-alpha.1+**（`agent/created`） | **0.7.1+** | 双事件注册（`agent/created` + 旧 `agent/session-start`）；`systemPrompt` 按作用域服务解析 |
| **0.1.5-rc.2**（实测通过） | 0.7.0 – 0.7.1 | 统一锚点生命周期、AST/LSP 拆分、设置卡片、卡片全景图 |
...
- 构建/测试 SDK 基线：`0.1.6-alpha.1`；实测环境 dsh `0.1.5-rc.2`，目标适配 `0.1.6-alpha.1`（#134）。
