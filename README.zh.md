...
**面向 Agent 的现代编辑范式。** 内容地址（3 字符 hash）与行号搭配为 `<line>#<hash>`：编辑文件的一部分时，行号部分按 `Shift:` 块描述的累计偏移更新，下一次编辑用 `newLine#oldHash` 即可，无需重新读取。模型按行**是什么**（hash）+ **当前在哪**（line）来定位，而不是按它之前在第几行。
