...
describe("invariant 3 — exclusivity", () => {
	...
	it("_mergeServedRows enforces single ownership: last write wins", () => {
		const current: (string | null)[] = ["X", "Y", null];
		const rows: ServedEntry[] = [{ position: 2, anchor: "X" }];
		const merged = _mergeServedRows(current, rows);
		expect(merged).toEqual([null, "Y", "X"]); // old X@0 nulled, X lives at 2
	});

	it("migrateServedAfterEdit pairs repeated anchors positionally, then the mirror purges", () => {
		...
		// Positional pairing can leave the same value live twice; the mirror's
		// single-ownership purge resolves it (last position wins):
		expect(_mergeServedRows(migrated, [])).toEqual([null, "S"]); // trailing nulls trim
	});
});
